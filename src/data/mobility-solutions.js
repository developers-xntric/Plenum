export const md_solutions = [
    {
        title: "Dynamics CRM Application",
        icon: "/service/MD/icon1.svg",
        description: "Empower your sales and customer service teams with an intuitive, mobile-friendly CRM solution tailored to your unique business processes.",
    },
    {
        title: "Workflow Approvals Application",
        icon: "/service/MD/icon2.svg",
        description: "Accelerate decision-making with a seamless approval workflow application—fully customizable and built for on-the-go responsiveness.",
    },
    {
        title: "Retail Management Application",
        icon: "/service/MD/icon3.svg",
        description: "Digitize your retail operations with a robust mobile solution that simplifies store processes, inventory, and customer interactions.",
    },
    {
        title: "Warehouse Management Application",
        icon: "/service/MD/icon4.svg",
        description: "Streamline warehouse tasks from inventory tracking to order fulfillment with a dynamic, user-friendly mobile interface.",
    },
    {
        title: "Timesheet Tracking App",
        icon: "/service/MD/icon5.svg",
        description: "Enable employees to log work hours, manage shifts, and submit timesheets effortlessly—right from their mobile devices.",
    },
    {
        title: "Expense Management App",
        icon: "/service/MD/icon6.svg",
        description: "Take control of business expenses with a powerful mobile app for easy receipt uploads, expense tracking, and real-time reporting.",
    },
    {
        title: "Loyalty Program App",
        icon: "/service/MD/icon7.svg",
        description: "Drive customer engagement and retention with a customizable loyalty management app, fully integrated with Microsoft Dynamics.",
    },
    {
        title: "Want a custom solution for your company?",
        icon: "/service/MD/icon7.svg",
        description: "Sublime is easy to use and totally customizable to your business needs.",
    },
]

export const bc_solution = [
    {
        title: "Comprehensive Business Management",
        icon: "/service/MD/icon1.svg",
        para: 'Manage finance, operations, and projects seamlessly within a unified ERP system.',
    },
    {
        title: "Real-Time Insights",
        icon: "/service/MD/icon2.svg",
        para: 'Access up-to-date data and analytics to make faster, more informed business decisions.',
    },
    {
        title: "Streamlined Financial Management",
        icon: "/service/MD/icon3.svg",
        para: 'Simplify budgeting, forecasting, and compliance with powerful financial tools.',
    },
    {
        title: "Scalability to Grow",
        icon: "/service/MD/icon4.svg",
        para: 'Expand effortlessly with solutions that adapt to your company’s growth and complexity.',
    },
    {
        title: "Cloud Flexibility & Security",
        icon: "/service/MD/icon5.svg",
        para: 'Leverage secure, cloud-based infrastructure that ensures flexibility, uptime, and data protection.',
    },
]
