...
export default function TestimonialCard({ quote, author, position, socialLink }) {
  return (
    <div className="bg-[#EFEFEF] p-4 md:p-6 relative transition-all duration-300 ease-in-out h-full flex flex-col justify-between">
      <div className="flex-grow">
        <blockquote className="min-h-[120px] flex items-start">
          <p className="text-secondary font-['Archivo'] opacity-75 w-full xl:max-w-[90%] 2xl:max-w-[78%]">{`${quote}`}</p>
        </blockquote>
      </div>
      <div className="flex items-center justify-between mt-6">
        <div className="-space-y-3">
          <p className="leading-[55px] font-['Archivo'] text-[18px] lg:text-[20px] text-secondary font-semibold home-section-headings">{author}</p>
          <p className="text-[12px] lg:text-sm text-secondary font-['Archivo'] opacity-75">{position}</p>
        </div>
         <Link href={socialLink} target="_blank" rel="noopener noreferrer">
          <svg xmlns="http://www.w3.org/2000/svg" width="29" height="29" viewBox="0 0 29 29" fill="none" className="lg:block hidden">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none" className="lg:hidden block">
